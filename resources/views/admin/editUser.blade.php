...
<body class="bg-gray-800 text-white">
    <x-app-layout>
        ...
        <div class="flex items-center justify-center pt-10">
            <form action="{{ route('user.update', ['id' => $user->id]) }}" method="post" class="max-w-md mx-auto bg-gray-800 p-8 rounded-md shadow-md">
                @csrf
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="name" name="name" value="{{ $user->name }}" class="bg-gray-700 border border-gray-600 text-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nama petugas" required>
                </div>
                <div class="mb-5">
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-300">Email</label>
                    <input type="email" id="email" name="email" value="{{ $user->email }}" class="bg-gray-700 border border-gray-600 text-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="nama@gmail.com" required>
                </div>
                <div class="mb-5">
                ...
                <div class="mb-5">
                    ...
                    <select id="role" name="role" class="bg-gray-700 border border-gray-600 text-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                        <option value="petugas_sosial" {{ $user->role === 'petugas_sosial' ? 'selected' : '' }}>Petugas Sosial</option>
                        <option value="petugas_infrastruktur" {{ $user->role === 'petugas_infrastruktur' ? 'selected' : '' }}>Petugas Infrastruktur</option>
                        <option value="petugas_lingkungan" {{ $user->role === 'petugas_lingkungan' ? 'selected' : '' }}>Petugas Lingkungan</option>
                    </select>
                </div>
